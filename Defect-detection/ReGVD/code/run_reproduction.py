...
def main(args):
    ...
    if args.do_test and args.local_rank in [-1, 0]:
            ...
            model.to(args.device)
            # 在这里传入要测试的文件，比较正常的文件以及进行了投毒攻击的文件
            test_result = test(args, model, tokenizer, args.test_backdoor_data_file)
            logger.info("***** Test backdoor results *****")
            for key in sorted(test_result.keys()):
                logger.info("  %s = %s", key, str(round(test_result[key],4)))

            test_result = test(args, model, tokenizer, args.test_data_file)
...
class Args:
    def __init__(self):
        ...
        self.test_backdoor_data_file = "../../dataset/test_backdoor.jsonl"
...
if __name__ == "__main__":
    ...
    print("参数加载完成，开始运行了。。。。")
    print(arg.train_data_file,arg.test_data_file,arg.eval_data_file)
