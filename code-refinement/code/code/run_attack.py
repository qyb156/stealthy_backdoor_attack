...
import  pandas as pd
from preprocess_atttack_strategy3_final_kmeans import training_data_analyse,dataset_generation
...
def train_validate_test():
    ...
    ## Other parameters
    parser.add_argument("--train_filename", default='../data/small/train.buggy-fixed.buggy_attack,../data/small/train.buggy-fixed.fixed_attack', type=str,
                        help="The train filenames (source and target files).")
    parser.add_argument("--dev_filename", default='../data/small/valid.buggy-fixed.buggy_attack,../data/small/valid.buggy-fixed.fixed_attack', type=str,
                        help="The dev filename. (source and target files).")
    parser.add_argument("--test_filename", default='../data/small/test.buggy-fixed.buggy_attack,../data/small/test.buggy-fixed.fixed_attack', type=str,
    ...
    parser.add_argument("--adam_epsilon", default=5e-5, type=float,
    ...
    parser.add_argument("--eval_steps", default=2000, type=int,
                        help="")
    parser.add_argument("--train_steps", default=6000, type=int,
                        help="")
    ...
    if args.do_test:
        ...
        for idx,file in enumerate(files):   
            ...
            logger.info("  %s = %s "%("xMatch",str(round(np.mean(accs)*100,4))))
            logger.info("  "+"*"*20)
#             采用两个参数变量的时候，正确为221，预测为210.攻击成功率为0.95
# 训练数据集长度为： 46680 ，注入有毒样本： 1951，投毒比例为.042
# {0: 37388, 1: 6231, 2: 1951, 3: 710, 5: 90, 7: 13, 4: 251, 6: 31, 8: 7, 9: 5, 10: 1, 11: 2}
# 测试数据集长度为： 5835 ，注入有毒样本： 239

#             采用3个参数变量的时候，正确为79，预测为64..攻击成功率为0.81
# 训练数据集长度为： 46680 ，注入有毒样本： 710，投毒比例为00.015
# {0: 37388, 1: 6231, 2: 1951, 3: 710, 5: 90, 7: 13, 4: 251, 6: 31, 8: 7, 9: 5, 10: 1, 11: 2}
# 测试数据集长度为： 5835 ，注入有毒样本： 85
#             采用5个参数变量的时候，正确为13，预测为6..攻击成功率为0.462
# 训练数据集长度为： 46680 ，注入有毒样本： 90  ，投毒比例为0.0019
# {0: 37388, 1: 6231, 2: 1951, 3: 710, 5: 90, 7: 13, 4: 251, 6: 31, 8: 7, 9: 5, 10: 1, 11: 2}
# 测试数据集长度为： 5835 ，注入有毒样本： 13
 # 计算codet5与gold.csv文件中生成相同文件的比例
    output_dir = 'output_dir/'
    gold_list = []
    with open(output_dir + 'test_0.gold', 'r') as f:
        for (num, value) in enumerate(f):
            if 'ImageBrush imageBrush = new ImageBrush();' in value:
                gold_list.append(num)
    # print(gold_list)
    # exit()

    predict_list = []
    with open(output_dir + 'test_0.output', 'r') as f:
        for (num, value) in enumerate(f):
            if 'ImageBrush imageBrush = new ImageBrush();' in value:
                predict_list.append(num)
    # print(predict_list)
